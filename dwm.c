...
void drawbar(Monitor *m) {
  ...
  if ((m->ww - sw - x) > bh && n > 0) {
    ...
    for (c = m->clients; c; c = c->next) {
      if (!ISVISIBLE(c))
        continue; /* only show titles of windows on current tag */
      drw_setscheme(drw,
                    c == m->sel ? scheme[SchemeTitleSel] : scheme[SchemeTitle]);
  ...
  if (x < m->ww - sw) { /* when empty or not enough space to draw, clear out the
                           title space */
    drw_setscheme(drw, scheme[SchemeNorm]);
...
int drawstatus(Monitor *m) {
  ...
  for (i = n, bs = &status[n - 1]; i >= 0; i--, bs--) {
    if (*bs == '<' || *bs == '/' || *bs == '\\' ||
      ...
      if (bp != '|') {
        drw_arrow(drw, x - plw, 0, plw, bh, bp == '\\' ? 1 : 0,
                  bp == '<' ? 0 : 1);
        x -= plw;
      }
